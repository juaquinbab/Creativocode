...
router.get('/status', async (_req, res) => {
  try {
    ...
    const ia = toBool(data?.cliente11?.IA); // si no existe, será false
    res.json({ ia });
  } catch (e) {
...
router.post('/toggle', async (_req, res) => {
  try {
    const data = await read();
    if (!data.cliente11|| typeof data.cliente11 !== 'object') data.cliente11 = {};

    const current = toBool(data.cliente11.IA);
    const next = !current;
    data.cliente11.IA = next;
