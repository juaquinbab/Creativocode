...
router.get('/status', async (_req, res) => {
  try {
    ...
    const ia = toBool(data?.cliente17?.IA); // si no existe, será false
    res.json({ ia });
  } catch (e) {
...
router.post('/toggle', async (_req, res) => {
  try {
    const data = await read();
    if (!data.cliente17 || typeof data.cliente17 !== 'object') data.cliente17 = {};

    const current = toBool(data.cliente17.IA);
    const next = !current;
    data.cliente17.IA = next;
    await write(data);
    res.json({ ia: next });
  } catch (e) {
