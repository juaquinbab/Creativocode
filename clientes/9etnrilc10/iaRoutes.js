...
router.get('/status', async (_req, res) => {
  try {
    ...
    const ia = toBool(data?.cliente10?.IA); // si no existe, será false
    res.json({ ia });
  } catch (e) {
...
router.post('/toggle', async (_req, res) => {
  try {
    const data = await read();
    if (!data.cliente10|| typeof data.cliente10 !== 'object') data.cliente10 = {};

    const current = toBool(data.cliente10.IA);
    const next = !current;
    data.cliente10.IA = next;
