...
router.get('/status', async (_req, res) => {
  try {
    ...
    const ia = toBool(data?.cliente3?.IA); // si no existe, será false
    res.json({ ia });
  } catch (e) {
...
router.post('/toggle', async (_req, res) => {
  try {
    const data = await read();
    if (!data.cliente3 || typeof data.cliente3 !== 'object') data.cliente3 = {};

    const current = toBool(data.cliente3.IA);
    const next = !current;
    data.cliente3.IA = next;
