...
router.get('/status', async (_req, res) => {
  try {
    ...
    const ia = toBool(data?.cliente15?.IA); // si no existe, será false
    res.json({ ia });
  } catch (e) {
...
router.post('/toggle', async (_req, res) => {
  try {
    const data = await read();
    if (!data.cliente4|| typeof data.cliente15 !== 'object') data.cliente15 = {};

    const current = toBool(data.cliente15.IA);
    const next = !current;
    data.cliente15.IA = next;
