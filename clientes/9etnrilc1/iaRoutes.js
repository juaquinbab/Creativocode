...
router.get('/status', async (_req, res) => {
  try {
    ...
    const ia = toBool(data?.cliente1?.IA); // si no existe, será false
    res.json({ ia });
  } catch (e) {
...
router.post('/toggle', async (_req, res) => {
  try {
    const data = await read();
    if (!data.cliente4|| typeof data.cliente1 !== 'object') data.cliente1 = {};

    const current = toBool(data.cliente1.IA);
    const next = !current;
    data.cliente1.IA = next;
