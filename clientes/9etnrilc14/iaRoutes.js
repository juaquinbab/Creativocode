...
router.get('/status', async (_req, res) => {
  try {
    ...
    const ia = toBool(data?.cliente14?.IA); // si no existe, será false
    res.json({ ia });
  } catch (e) {
...
router.post('/toggle', async (_req, res) => {
  try {
    const data = await read();
    if (!data.cliente4|| typeof data.cliente14 !== 'object') data.cliente14 = {};

    const current = toBool(data.cliente14.IA);
    const next = !current;
    data.cliente14.IA = next;
