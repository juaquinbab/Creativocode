...
router.get('/status', async (_req, res) => {
  try {
    ...
    const ia = toBool(data?.cliente6?.IA); // si no existe, será false
    res.json({ ia });
  } catch (e) {
...
router.post('/toggle', async (_req, res) => {
  try {
    const data = await read();
    if (!data.cliente6 || typeof data.cliente6 !== 'object') data.cliente6 = {};

    const current = toBool(data.cliente6.IA);
    const next = !current;
    data.cliente6.IA = next;
