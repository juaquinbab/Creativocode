...
router.get('/status', async (_req, res) => {
  try {
    ...
    const ia = toBool(data?.cliente13?.IA); // si no existe, será false
    res.json({ ia });
  } catch (e) {
...
router.post('/toggle', async (_req, res) => {
  try {
    const data = await read();
    if (!data.cliente13|| typeof data.cliente13 !== 'object') data.cliente13 = {};

    const current = toBool(data.cliente13.IA);
    const next = !current;
    data.cliente13.IA = next;
