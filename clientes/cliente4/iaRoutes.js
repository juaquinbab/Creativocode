...
router.get('/status', async (_req, res) => {
  try {
    ...
    const ia = toBool(data?.cliente4?.IA); // si no existe, será false
    res.json({ ia });
  } catch (e) {
...
router.post('/toggle', async (_req, res) => {
  try {
    const data = await read();
    if (!data.cliente4 || typeof data.cliente4 !== 'object') data.cliente4 = {};

    const current = toBool(data.cliente4.IA);
    const next = !current;
    data.cliente4.IA = next;
