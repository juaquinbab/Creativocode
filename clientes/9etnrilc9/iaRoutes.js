...
router.get('/status', async (_req, res) => {
  try {
    ...
    const ia = toBool(data?.cliente9?.IA); // si no existe, será false
    res.json({ ia });
  } catch (e) {
...
router.post('/toggle', async (_req, res) => {
  try {
    const data = await read();
    if (!data.cliente9 || typeof data.cliente9 !== 'object') data.cliente9 = {};

    const current = toBool(data.cliente9.IA);
    const next = !current;
    data.cliente9.IA = next;
