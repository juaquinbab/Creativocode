...
router.get('/status', async (_req, res) => {
  try {
    ...
    const ia = toBool(data?.cliente16?.IA); // si no existe, será false
    res.json({ ia });
  } catch (e) {
...
router.post('/toggle', async (_req, res) => {
  try {
    const data = await read();
    if (!data.cliente16 || typeof data.cliente16 !== 'object') data.cliente16 = {};

    const current = toBool(data.cliente16.IA);
    const next = !current;
    data.cliente16.IA = next;
    await write(data);
    res.json({ ia: next });
  } catch (e) {
